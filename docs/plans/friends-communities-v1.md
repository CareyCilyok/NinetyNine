...
| 2026-04-11 | S3.5 note: the `relationship >= fieldAudience` check described in the Audience tier semantics section has the operands inverted for the given enum ordering. Implementation uses `(int)viewerRelationship <= (int)fieldAudience` — mathematically equivalent to "the viewer's floor is at or below the field's audience tier". The new `ViewerRelationship` enum mirrors `Audience`'s numeric space (`Self=0, Friend=1, CommunityMember=2, Public=3`) plus an `Anonymous = -1` sentinel. No scope change; this is a documentation clarification, not a behavior change. | Claude (during S3.5 implementation) |
